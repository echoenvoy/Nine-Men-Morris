...
static int readIntInput(int *outValue) {
    char buffer[64];
    char *end = NULL;
    long value = 0;
...
static int readCharInput(char *outChar) {
    char buffer[64];
    char *p = NULL;
...
void isValidCharch(char* sj){
   while(!isalpha((unsigned char)*sj)) {
...
int checkAndHandleMoulin(int iplace, int *itake, char adversarySymbol, int *adversaryPawns) {
    if (moulin(iplace) ) {
        ...
        if (hasNonMoulinPawns) {
            ...
            for (;;) {
                if (!readIntInput(itake)) {
                    printf("Erreur : Vous devez entrer un nombre !\n");
                    continue;
                }
                if (*itake < 0 || *itake >= SIZE) {
                ...
                if (!(board[*itake] == adversarySymbol && !moulin(*itake))) {
            ...
            for (;;) {
                ...
                if (!(board[*itake] == adversarySymbol)) {
...
void TourDePlacement(char joueurSymbol, char adversarySymbol, int joueurNum, int *adversaryPawns) {
    ...
    do {
        ...
        if (!readIntInput(&iplace)) {
            printf("Erreur : Vous devez entrer un nombre !\n");
            continue;
        }
        if (!isValid(iplace) || iplace < 0 || iplace > 23) { // Vérifie si l'entrée est valide
            printf("Erreur : Position invalide. Veuillez choisir une position entre 0 et 23 !\n");
            continue;
        }
        break;
...
void TourDeMvt(char joueurSymbol, char adversarySymbol, int joueurNum,int joueurPawns, int *adversaryPawns) {
    ...
    do {
        ...
        if (!readIntInput(&ideplace)) { // Tente de lire un entier
           printf("Erreur : Vous devez entrer un nombre !\n");
           continue;
        }
        if (ideplace < 0 || ideplace >= SIZE) {
            printf("Erreur : Position invalide. Veuillez choisir une position entre 0 et 23 !\n");
            continue;
        }
...
int is_win() {
    if (nbrspions1 <= 2 || (isBlocked(sj1) && nbrspions1 != 3)) {//verifie si un joueur ne peut plus bouger ou si l'un des joueurs n’a que deux pions
        ...
        displayBoard();
        if (sj2 == 'm'){
            printf(GREEN"La machine (%c) a gagne !\n"RESET,sj2);
        }
        else {
...
static int getPawnCountForSymbol(char sj) {
    if (sj == sj1) {
        return nbrspions1;
    }
    if (sj == sj2) {
...
int isBlocked(char sj) {
    int pawnCount = getPawnCountForSymbol(sj);
    if (allowFlying && pawnCount == 3) {
        for (int i = 0; i < SIZE; i++) {
            if (board[i] == 'O') {
                return 0;
            }
